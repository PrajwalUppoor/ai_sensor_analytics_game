...
function AchievementToast({ach,onDone}){
  ...
  return (
    <div style={{position:"fixed",top:16,right:16,zIndex:10000,background:"#12121f",border:"1.5px solid #f7b731",borderRadius:16,padding:"14px 18px",display:"flex",gap:12,alignItems:"center",boxShadow:"0 8px 32px #f7b73155",minWidth:260,maxWidth:320}}>
...
function TimerRing({secs,total,color}){
  ...
  return (
    <div style={{position:"relative",width:54,height:54,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
      ...
      <span style={{fontFamily:"Orbitron,monospace",fontWeight:900,fontSize:13,color:urg?"#fc5c65":color}}>{secs}</span>
    </div>
  );
}
...
function QuizHome({onStart}){
  return (
    <div style={{textAlign:"center",padding:"44px 20px 60px",position:"relative",zIndex:1}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,maxWidth:640,margin:"0 auto 36px"}}>
        {MISSIONS.map((m)=>(
          <div key={m.id} style={{padding:"14px 6px",borderRadius:12,border:`1.5px solid ${m.color}2a`,background:`${m.color}0a`,color:m.color}}>
...
function MissionSelect({completedMissions,missionStars,score,lives,streak,onSelect,onBack}){
  return (
    <div style={{padding:"18px 16px",position:"relative",zIndex:1}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
        <div>
          <button onClick={onBack} style={{background:"none",border:"none",color:"#555",cursor:"pointer",marginBottom:10,fontSize:10}}>← MAIN MENU</button>
        ...
        {MISSIONS.map((m,idx)=>{
          ...
          return (
            <div key={m.id} onClick={()=>!locked&&onSelect(m)}
              style={{padding:"15px 16px",borderRadius:13,border:`1.5px solid ${done?m.color:locked?"#161616":`${m.color}44`}`,background:done?`${m.color}10`:locked?"rgba(0,0,0,0.2)":"rgba(255,255,255,0.025)",cursor:locked?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:14,transition:"all 0.18s",opacity:locked?0.3:1}}>
...
function QuestionScreen({mission,questionIdx,lives,streak,score,onAnswer,onBack}){
  ...
  return (
    <div style={{padding:"15px",position:"relative",zIndex:1}}>
      ...
      {feedback&&(
        <div style={{position:"fixed",inset:0,background:"rgba(5,5,12,0.92)",zIndex:9000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
          <div style={{fontSize:64,marginBottom:10}}>{feedback.timeout?"⏰":feedback.correct?"✅":"❌"}</div>
...
function QuizGameOver({score,onRestart}){
  return (
...
function FinalScreen({score,missionStars,unlockedAchievements,onRestart}){
  ...
  return (
    <div style={{padding:"28px 18px 60px",position:"relative",zIndex:1,maxWidth:600,margin:"0 auto"}}>
...
export default function QuizGame({ score, setScore, onBackToMenu, onAchievement }){
  ...
  const checkAch=(state)=>{
    ...
    if(newOnes.length){
      ...
      newOnes.forEach(a => onAchievement && onAchievement(a));
    }
  };
  ...
  }, [score, streak, maxStreak, lives, correctCt, spdEver, mWrong, qIdx, activeMission, completed, flawless, seenAch, setScore]);
  ...
  return (
    <div style={{position:"relative"}}>
      ...
      {screen==="home"    &&<QuizHome onStart={()=>setScreen("missions")}/>}
      {screen==="missions"&&<MissionSelect completedMissions={completed} missionStars={stars} score={score} lives={lives} streak={streak} onSelect={startMission} onBack={onBackToMenu}/>}
      ...
      {screen==="gameover"&&<QuizGameOver score={score} onRestart={restart}/>}
